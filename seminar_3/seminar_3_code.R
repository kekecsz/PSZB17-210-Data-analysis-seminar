...
# Ennek az adatbázisnak az adatait fogjuk ábrázolni. Az ábrázoláshoz a **ggplot2** nevű csomagot használjuk majd. Töltsd be ezt a csomagot! A tidyverse csomag tartalmazza a ggplot2-t, így az alábbi kódban ezen keresztül töltöm be a ggplot2-t. Így a %>%operatárot is használni tudjuk és egyéb dplyer funkciókat.	
...
# Nézd hogyan lehet a pipe %>% operátort használni ahhoz, hogy meg a ggplot funkciót a movies adatbázisra alkalmazzuk.	
...
# Az alábbi éldán láthatjuk, hogy hogyan tudunk egy új változót bevonni a megjelenítésben. Ebben az esetben egy másik kritikus értékelést jelenítünk meg színekkel. Mivel a geom_point-ot használjuk, ez a pontok színét fogja befolyásolni, de ha más geomot használnák, azokban is hatna ez a színezésre, hiszen az aes() általános aesthetics részben specifikáltuk.	
...
# Nem kell mindig kiírnunk a teljes ábra kódot amikor valam,i új elemet szeretnénk hozzáadni az ábrázoláshoz. Ha az ábrát korábban elmentettük objektumként, akkor az objektumhoz + jellel hozzáadhatjuk az új elemeket.	
...
# A színeket be lehet írni angolul. Egy részletesebb guide erről:	
...
# - Nézd meg, hogy a műfaj (genre) releváns-e ebben az előbbi összefüggésnél (a műfaj változó is jelenjen meg valahogy az ábrán).	
...
# - Vizsgáld meg kritikusok által adott értékelás (critics_score) eloszlását	
# - Tetszőleges geomot használhatsz. A ggplot2 cheatsheet segíthet kitalálni, melyik a legjobb geom erre a célra. A critcs_score egy folytonos (continuous) változó:	
# https://rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf)	
# - Az ábrán az is legyen látható, hogy az eloszlás hogyan különbözik azoknál a filmeknél amiket jelöltek a legjobb film oszkárdíjra (best_pic_nom). Ehhez használj tetszőleges aes()-t, pl.: color, fill, linetype, size. A ggplot2 cheatsheet segít hogy az általad választott geomnál melyik a releváns aes()	
