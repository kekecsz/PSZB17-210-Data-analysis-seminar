...
library(quantmod) # for actual foreign exhange rates	
library(measurements) # for converting measuement units	
library(ggfortify) # for diagnostic plots in ggplot	
...
# Aktuális Forint USD átváltás lekérése a Yahoo Finance-rol	
usd_in_huf <- 	
  getQuote(paste0("USD", "HUF","=X"))["Last"] %>% 	
  as.numeric()	
	
data_house = data_house %>% 	
  mutate(price_mill_HUF = (price * usd_in_huf)/1000000,	
         sqm_living = conv_unit(sqft_living, "ft2", "m2"),	
         sqm_lot = conv_unit(sqft_lot, "ft2", "m2"),	
         sqm_above = conv_unit(sqft_above, "ft2", "m2"),	
         sqm_basement = conv_unit(sqft_basement, "ft2", "m2"),	
         sqm_living15 = conv_unit(sqft_living15, "ft2", "m2"),	
         sqm_lot15 = conv_unit(sqft_lot15, "ft2", "m2")	
         )	
...
mod_house4 = lm(price_mill_HUF_transformed ~ sqm_living + grade, 	
                data = data_house_nooutliers)	
	
autoplot(mod_house4, which = 3)	
