...
# Jelen kutasban csak az erdekel minket hogy a testsuly befolyasolja-e az elvett szendvicsek szamat, es ha igen, mennyire. Az iskolai osztalyok hatása nem része a fő kutatási kérdésnek, és még ha az is lenne, az informaciot amit ezekrol az iskolai osztalyokrol szerzunk **nem tudnank altalanositani mas iskolakban**. Nem lenne sok értelme megtudni, hogy mi a hatása annak ha valaki a "class 1"-be jár, amikor a regressziós modellünket új mintán szeretnenk bejoslasra hasznalni egy uj iskolaban, hiszen a tobbi iskolaban mas osztalyok vannak, amiknek velhetoen masok a karakterisztikai. Szoval az iskolai osztaly ebben az esetben egy "zavaro tenyezo" (**nuisance variable**). Vagyis ezt a class valtozot nem szeretnenk figyelembe venni a regresszios egyenletben, hiszen akkor mas iskolakban nem tudnak felhasznalni az egyenletet.	
...
# A fenti abra alapjan arra lehet kovetkeztetni hogy a **data_bully_slope** adatbázisban az iskolai osztaly egy olyan random hatas, ami mind a regresszios egyenes intercept-jet, mind a meredekseget (slope) befolyasolja. Ezert normalis esetben csak a random slope modellt illesztenenk. A tobbi modell csak demonstracios celbol epitjuk, hogy osszehasonlitsuk azok formulait es bejoslo erejet a random slope modellevel.	
...
# 1. Olvassuk be az adatokat (egy .csv kiterjesztésű file-ból). Az adatokat az alábbi linkről tölthetjük le: "https://raw.githubusercontent.com/kekecsz/PSZB17-210-Data-analysis-seminar/master/seminar_09/home_sample_5.csv".	
