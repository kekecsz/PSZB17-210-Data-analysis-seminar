...
```{r two categorical variable plots 1}
...
```{r two categorical variable plots 2}
...
```{r two categorical variable plots 3}
...
# az y tengelyen megvaltoztathatjuk a beosztast a scale_y_continuous(breaks = c(...))) funkcio hozzaadasaval 
...
```{r categorical vs numeric plot 1}
...
```{r categorical vs numeric plot 3}
...
```{r two numerical variables plot 1}
...
- Terjunk vissza a *orai_adat_harmashangulat_1* (amiben a rossz hangulatuak is benne vannak). Most a *hangulat_kat_1* valtozo es a *valasztott_szam* valtozo kapcsolatat vizsgaljuk majd. Eloszor is jelenitsd meg hogy a *hangulat_kat_1* egyes kategoriaiban atlagosna mekkora szamot valasztottak az emberek (*valasztott_szam* valtozo).
...
- Milyen a kapcsolat az elso orai es a harmadik orai hangulat kozott (*hangulat_1* es *hangulat_3* valtozok? Hatarozd mega korrelacios egyutthatot a ket valtozo kozott.

- Abrazold a *hangulat_1* es *hangulat_3* valtozok kozotti kapcsolatot egy abraval.
