...
# A kategorikus valtozokat gyakran oszlopdiagrammal (geom_bar) abrazoljuk, mig a numerikus valtozokat inkabb dotplot , histogram, vagy density plot segitsegevel szoktuk abrazolni.	
...
# A mutate() es a replace() funkciok hasznalataval cserelhetunk ki ertekeket mas ertekekre. Azt, hogy ilyenkor hianyzo adatra (NA), vagy egy masik, valoszinu ertekre kell megvaltoztatni az erteket, a szituaciotol fogg. Altalaban a biztosabb megoldas ha hianyzo adatnak jeloljuk a kerdeses erteket (NA), de ez sok adatveszteshez vezethet. Ha eleg valszinu hogy mi a helyes valasz, beirhatjuk, DE minden javitast fel kell tuntetni a kutatasi jelentesben (es a ZH soran is), hogy az olvaso szamara tiszta legyen, hogy itt egy adathelyettesites vagy kizaras tortent!	
...
# hasznalhatnak meg az alabbiakat is arra, hogy megbizonyosodjunk abban, hogy sikeres volt a csere	
...
# Az elso oran kertunk egy tippet arrol, hogy az egyes emberek milyen jegyet fognak kapni a kurzus vegen az adatelemzes oran. Mivel itt szamban kertuk a valaszt, ez jelenleg egy numerikus valtozo, de csinaljunk most belole egy kategorikus valtozot, mert osszesen harom fajta tipp erkezett: 3 - "kozepes", 4 - "jo", 5 - "kivalo"	
...
# Most vizsgaljuk meg azt, hogy az, hogy az emberek mennyit alszanak altalaban hogyan fugg ossze azzal, hogy mennyit hasznaljak a szocialis mediat.	
...
# Ennel a megjelenitesnel fontos hogy ha az egyes megfigyelesek keves megfigyelesbol allnak, az abra megteveszto lehet, mert az abra nem jelzi a megfigyelesek szamat es igy azt, hogy milyen biztosak lehetunk az eredmenyben. Ilyen esetekben az egyik kategoriat ki lehet venni az abrarol, vagy a szmossagot es a reszaranyt abrazolo abrakat egymas mellet lehet bemutatni, hogy igy kiegeszitsek egymast. Ehhez hasznalhatjuk a grid.arrange() funkciot.	
...
# a theme(legend.position) es a guides() funciok hasznalataval kontrollalhatjuk hogy hol es hogyan jelenjen meg a jelmagyarazat az abran	
...
# Vizsgaljuk meg a magassag osszefuggeset azzal, hogy ez emberek mit tippeltek, milyen jegyet fognak kapni az elso oran.	
