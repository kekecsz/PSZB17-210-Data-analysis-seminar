...
data_house %>% 
  ggplot() +
  aes(x = grade, y = price_mill_HUF) +
  geom_point()

cor1 = cor.test(data_house$price_mill_HUF, data_house$grade)
cor1 

# 4. Illessz egy egyszeru regresszios modellt amelyben a lakas arat (price) josolod be a lakas minosegevel (grade) 

mod_1 = lm(price_mill_HUF ~ grade, data = data_house)
summary(mod_1)
...
mod_2 = lm(price_mill_HUF ~ grade + sqm_living + condition + yr_built, data = data_house)
summary(mod_2)
